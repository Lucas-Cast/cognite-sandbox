from industrial_model import Engine, PaginatedResult
from industrial_model.queries import build_query_statement
from industrial_model.view_client import ViewClient

from .filters import ProductionOutputFilter
from .models import ProductionOutput, ProductionOutputAggregation
from .types import (
    ProductionOutputAggregationProperty,
    ProductionOutputGroupByProperty,
    ProductionOutputIncludeProperty,
    ProductionOutputQueryProperty,
)

_RELATION_PROPERTIES: frozenset[str] = frozenset({"asset", "material", "uom"})


class ProductionOutputClient(
    ViewClient[
        ProductionOutput,
        ProductionOutputAggregation,
        ProductionOutputFilter,
        ProductionOutputQueryProperty,
        ProductionOutputGroupByProperty,
        ProductionOutputAggregationProperty,
    ]
):
    def __init__(self, engine: Engine) -> None:
        super().__init__(engine, ProductionOutput, ProductionOutputAggregation)

    def query(
        self,
        filters: ProductionOutputFilter | None = None,
        *,
        include: list[ProductionOutputIncludeProperty] | None = None,
        limit: int = 1000,
        cursor: str | None = None,
    ) -> PaginatedResult[ProductionOutput]:
        return self._engine.query(
            build_query_statement(
                self._entity_cls,
                filters,
                exclude_relations=[
                    p for p in _RELATION_PROPERTIES if p not in (include or ())
                ],
                limit=limit,
                cursor=cursor,
            )
        )

    async def query_async(
        self,
        filters: ProductionOutputFilter | None = None,
        *,
        include: list[ProductionOutputIncludeProperty] | None = None,
        limit: int = 1000,
        cursor: str | None = None,
    ) -> PaginatedResult[ProductionOutput]:
        return await self._engine.query_async(
            build_query_statement(
                self._entity_cls,
                filters,
                exclude_relations=[
                    p for p in _RELATION_PROPERTIES if p not in (include or ())
                ],
                limit=limit,
                cursor=cursor,
            )
        )

    def query_all_pages(
        self,
        filters: ProductionOutputFilter | None = None,
        *,
        include: list[ProductionOutputIncludeProperty] | None = None,
        limit: int = 1000,
    ) -> list[ProductionOutput]:
        return self._engine.query_all_pages(
            build_query_statement(
                self._entity_cls,
                filters,
                exclude_relations=[
                    p for p in _RELATION_PROPERTIES if p not in (include or ())
                ],
                limit=limit,
            )
        )

    async def query_all_pages_async(
        self,
        filters: ProductionOutputFilter | None = None,
        *,
        include: list[ProductionOutputIncludeProperty] | None = None,
        limit: int = 1000,
    ) -> list[ProductionOutput]:
        return await self._engine.query_all_pages_async(
            build_query_statement(
                self._entity_cls,
                filters,
                exclude_relations=[
                    p for p in _RELATION_PROPERTIES if p not in (include or ())
                ],
                limit=limit,
            )
        )
