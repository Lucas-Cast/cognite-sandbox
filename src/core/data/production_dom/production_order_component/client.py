from industrial_model import Engine, PaginatedResult
from industrial_model.queries import build_query_statement
from industrial_model.view_client import ViewClient

from .filters import ProductionOrderComponentFilter
from .models import ProductionOrderComponent, ProductionOrderComponentAggregation
from .types import (
    ProductionOrderComponentAggregationProperty,
    ProductionOrderComponentGroupByProperty,
    ProductionOrderComponentIncludeProperty,
    ProductionOrderComponentQueryProperty,
)

_RELATION_PROPERTIES: frozenset[str] = frozenset({"order", "order|routing"})


class ProductionOrderComponentClient(
    ViewClient[
        ProductionOrderComponent,
        ProductionOrderComponentAggregation,
        ProductionOrderComponentFilter,
        ProductionOrderComponentQueryProperty,
        ProductionOrderComponentGroupByProperty,
        ProductionOrderComponentAggregationProperty,
    ]
):
    def __init__(self, engine: Engine) -> None:
        super().__init__(
            engine, ProductionOrderComponent, ProductionOrderComponentAggregation
        )

    def query(
        self,
        filters: ProductionOrderComponentFilter | None = None,
        *,
        include: list[ProductionOrderComponentIncludeProperty] | None = None,
        limit: int = 1000,
        cursor: str | None = None,
    ) -> PaginatedResult[ProductionOrderComponent]:
        return self._engine.query(
            build_query_statement(
                self._entity_cls,
                filters,
                exclude_relations=[
                    p for p in _RELATION_PROPERTIES if p not in (include or ())
                ],
                limit=limit,
                cursor=cursor,
            )
        )

    async def query_async(
        self,
        filters: ProductionOrderComponentFilter | None = None,
        *,
        include: list[ProductionOrderComponentIncludeProperty] | None = None,
        limit: int = 1000,
        cursor: str | None = None,
    ) -> PaginatedResult[ProductionOrderComponent]:
        return await self._engine.query_async(
            build_query_statement(
                self._entity_cls,
                filters,
                exclude_relations=[
                    p for p in _RELATION_PROPERTIES if p not in (include or ())
                ],
                limit=limit,
                cursor=cursor,
            )
        )

    def query_all_pages(
        self,
        filters: ProductionOrderComponentFilter | None = None,
        *,
        include: list[ProductionOrderComponentIncludeProperty] | None = None,
        limit: int = 1000,
    ) -> list[ProductionOrderComponent]:
        return self._engine.query_all_pages(
            build_query_statement(
                self._entity_cls,
                filters,
                exclude_relations=[
                    p for p in _RELATION_PROPERTIES if p not in (include or ())
                ],
                limit=limit,
            )
        )

    async def query_all_pages_async(
        self,
        filters: ProductionOrderComponentFilter | None = None,
        *,
        include: list[ProductionOrderComponentIncludeProperty] | None = None,
        limit: int = 1000,
    ) -> list[ProductionOrderComponent]:
        return await self._engine.query_all_pages_async(
            build_query_statement(
                self._entity_cls,
                filters,
                exclude_relations=[
                    p for p in _RELATION_PROPERTIES if p not in (include or ())
                ],
                limit=limit,
            )
        )
