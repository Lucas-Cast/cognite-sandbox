from industrial_model import Engine, PaginatedResult
from industrial_model.queries import build_query_statement
from industrial_model.view_client import ViewClient

from .filters import TimeSeriesSubserviceFilter
from .models import TimeSeriesSubservice, TimeSeriesSubserviceAggregation
from .types import (
    TimeSeriesSubserviceAggregationProperty,
    TimeSeriesSubserviceGroupByProperty,
    TimeSeriesSubserviceIncludeProperty,
    TimeSeriesSubserviceQueryProperty,
)

_RELATION_PROPERTIES: frozenset[str] = frozenset({"timeSeriesService"})


class TimeSeriesSubserviceClient(
    ViewClient[
        TimeSeriesSubservice,
        TimeSeriesSubserviceAggregation,
        TimeSeriesSubserviceFilter,
        TimeSeriesSubserviceQueryProperty,
        TimeSeriesSubserviceGroupByProperty,
        TimeSeriesSubserviceAggregationProperty,
    ]
):
    def __init__(self, engine: Engine) -> None:
        super().__init__(engine, TimeSeriesSubservice, TimeSeriesSubserviceAggregation)

    def query(
        self,
        filters: TimeSeriesSubserviceFilter | None = None,
        *,
        include: list[TimeSeriesSubserviceIncludeProperty] | None = None,
        limit: int = 1000,
        cursor: str | None = None,
    ) -> PaginatedResult[TimeSeriesSubservice]:
        return self._engine.query(
            build_query_statement(
                self._entity_cls,
                filters,
                exclude_relations=[
                    p for p in _RELATION_PROPERTIES if p not in (include or ())
                ],
                limit=limit,
                cursor=cursor,
            )
        )

    async def query_async(
        self,
        filters: TimeSeriesSubserviceFilter | None = None,
        *,
        include: list[TimeSeriesSubserviceIncludeProperty] | None = None,
        limit: int = 1000,
        cursor: str | None = None,
    ) -> PaginatedResult[TimeSeriesSubservice]:
        return await self._engine.query_async(
            build_query_statement(
                self._entity_cls,
                filters,
                exclude_relations=[
                    p for p in _RELATION_PROPERTIES if p not in (include or ())
                ],
                limit=limit,
                cursor=cursor,
            )
        )

    def query_all_pages(
        self,
        filters: TimeSeriesSubserviceFilter | None = None,
        *,
        include: list[TimeSeriesSubserviceIncludeProperty] | None = None,
        limit: int = 1000,
    ) -> list[TimeSeriesSubservice]:
        return self._engine.query_all_pages(
            build_query_statement(
                self._entity_cls,
                filters,
                exclude_relations=[
                    p for p in _RELATION_PROPERTIES if p not in (include or ())
                ],
                limit=limit,
            )
        )

    async def query_all_pages_async(
        self,
        filters: TimeSeriesSubserviceFilter | None = None,
        *,
        include: list[TimeSeriesSubserviceIncludeProperty] | None = None,
        limit: int = 1000,
    ) -> list[TimeSeriesSubservice]:
        return await self._engine.query_all_pages_async(
            build_query_statement(
                self._entity_cls,
                filters,
                exclude_relations=[
                    p for p in _RELATION_PROPERTIES if p not in (include or ())
                ],
                limit=limit,
            )
        )
