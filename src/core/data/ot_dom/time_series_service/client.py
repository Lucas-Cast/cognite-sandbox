from industrial_model import Engine, PaginatedResult
from industrial_model.queries import build_query_statement
from industrial_model.view_client import ViewClient

from .filters import TimeSeriesServiceFilter
from .models import TimeSeriesService, TimeSeriesServiceAggregation
from .types import (
    TimeSeriesServiceAggregationProperty,
    TimeSeriesServiceGroupByProperty,
    TimeSeriesServiceIncludeProperty,
    TimeSeriesServiceQueryProperty,
)

_RELATION_PROPERTIES: frozenset[str] = frozenset()


class TimeSeriesServiceClient(
    ViewClient[
        TimeSeriesService,
        TimeSeriesServiceAggregation,
        TimeSeriesServiceFilter,
        TimeSeriesServiceQueryProperty,
        TimeSeriesServiceGroupByProperty,
        TimeSeriesServiceAggregationProperty,
    ]
):
    def __init__(self, engine: Engine) -> None:
        super().__init__(engine, TimeSeriesService, TimeSeriesServiceAggregation)

    def query(
        self,
        filters: TimeSeriesServiceFilter | None = None,
        *,
        include: list[TimeSeriesServiceIncludeProperty] | None = None,
        limit: int = 1000,
        cursor: str | None = None,
    ) -> PaginatedResult[TimeSeriesService]:
        return self._engine.query(
            build_query_statement(
                self._entity_cls,
                filters,
                exclude_relations=[
                    p for p in _RELATION_PROPERTIES if p not in (include or ())
                ],
                limit=limit,
                cursor=cursor,
            )
        )

    async def query_async(
        self,
        filters: TimeSeriesServiceFilter | None = None,
        *,
        include: list[TimeSeriesServiceIncludeProperty] | None = None,
        limit: int = 1000,
        cursor: str | None = None,
    ) -> PaginatedResult[TimeSeriesService]:
        return await self._engine.query_async(
            build_query_statement(
                self._entity_cls,
                filters,
                exclude_relations=[
                    p for p in _RELATION_PROPERTIES if p not in (include or ())
                ],
                limit=limit,
                cursor=cursor,
            )
        )

    def query_all_pages(
        self,
        filters: TimeSeriesServiceFilter | None = None,
        *,
        include: list[TimeSeriesServiceIncludeProperty] | None = None,
        limit: int = 1000,
    ) -> list[TimeSeriesService]:
        return self._engine.query_all_pages(
            build_query_statement(
                self._entity_cls,
                filters,
                exclude_relations=[
                    p for p in _RELATION_PROPERTIES if p not in (include or ())
                ],
                limit=limit,
            )
        )

    async def query_all_pages_async(
        self,
        filters: TimeSeriesServiceFilter | None = None,
        *,
        include: list[TimeSeriesServiceIncludeProperty] | None = None,
        limit: int = 1000,
    ) -> list[TimeSeriesService]:
        return await self._engine.query_all_pages_async(
            build_query_statement(
                self._entity_cls,
                filters,
                exclude_relations=[
                    p for p in _RELATION_PROPERTIES if p not in (include or ())
                ],
                limit=limit,
            )
        )
